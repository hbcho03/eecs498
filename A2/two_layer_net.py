...
def nn_forward_backward(
    ...
    h1 = max(X*W1 + b1, 0)
    scores = h*W2 + b2
    softmax_loss = -log(exp(syi) / exp(sj).sum())
    '''
    num_train = X.shape[0]

    scores -= scores.max(dim=1).values.view(-1,1)  # (N,C)
    scores = scores.exp()
    probs = scores/scores.sum(dim=1).view(-1,1)
    label_probs = probs[range(num_train), y]
    loss = (-label_probs.log()).sum()
    loss /= num_train
...
def find_best_net(
    ...
    #############################################################################
    # Replace "pass" statement with your code
    learning_rates, hidden_sizes, regularization_strengths, \
      ...
      for hs in hidden_sizes:
        for rs in regularization_strengths:
          for lrd in learning_rate_decays:
            model = TwoLayerNet(3*32*32, hs,10,
            device=data_dict['X_train'].device, dtype=data_dict['X_train'].dtype)

            stats = model.train(data_dict['X_train'], data_dict['y_train'],
