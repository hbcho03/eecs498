...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        scores, cache_2 =  Linear.forward(out_1, self.params['W2'], self.params['b2'])
        ...
        dx, dw, db  = Linear.backward(dout, cache_2)
        grads['W2'] = dw + 2*self.params['W2']*self.reg
        grads['b2'] = db

        dx, dw, db  = Linear_ReLU.backward(dx, cache_1)
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i in range(self.num_layers):
          self.params['W{}'.format(i+1)] = torch.normal(0.0, weight_scale, (hidden_dims[i],hidden_dims[i+1]), device=device).type(dtype)
          self.params['b{}'.format(i+1)] = torch.zeros((hidden_dims[i+1],), device=device).type(dtype)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1,self.num_layers):
          out, cache_dict['cache_LR{}'.format(i)] = Linear_ReLU.forward(out,self.params['W{}'.format(i)],self.params['b{}'.format(i)])
          if self.use_dropout:
            out, cache_dict['cache_Dropout{}'.format(i)] = Dropout.forward(out,self.dropout_param)
        i+=1
        out, cache_dict['cache_L{}'.format(i)] = Linear.forward(out,self.params['W{}'.format(i)],self.params['b{}'.format(i)])
        scores = out
        ...
        dout, dw, db = Linear.backward(dout, cache_dict['cache_L{}'.format(i)])
        grads['W{}'.format(i)] = dw + 2*self.params['W{}'.format(i)]*self.reg
        grads['b{}'.format(i)] = db

        for i in range(1, self.num_layers)[::-1]:
          if self.use_dropout:
            dout = Dropout.backward(dout, cache_dict['cache_Dropout{}'.format(i)])
          dout, dw, db  = Linear_ReLU.backward(dout, cache_dict['cache_LR{}'.format(i)])
          grads['W{}'.format(i)] = dw + 2*self.params['W{}'.format(i)]*self.reg
          grads['b{}'.format(i)] = db
          loss += (self.params['W{}'.format(i)]**2).sum()*self.reg
...
def create_solver_instance(data_dict, dtype, device):
    ...
    solver = Solver(model, data_dict, num_epochs=20, device=device)
