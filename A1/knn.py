...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_train = x_train.clone().view((num_train,-1))
    x_test = x_test.clone().view((num_test,-1))
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
      dists[i] = ((x_train[i] - x_test)**2).sum(dim = 1).sqrt()
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_train = x_train.clone().view((num_train,-1))    #(N,D)
    x_test = x_test.clone().view((num_test,-1))       #(M,D)

    train_sqr = (x_train**2).sum(dim=1)   # (N,)
    test_sqr = (x_test**2).sum(dim=1)     # (M,)
    mul2 = x_train.mm(x_test.T)*2         # (N,M)
    dists = (train_sqr.view(-1,1) + test_sqr.view(1,-1) - mul2).sqrt()
...
def knn_cross_validate(
    ...
    for k in k_choices:
      ...
      for i in range(num_folds):
        train_x = torch.cat((x_train_folds[:i]+x_train_folds[i+1:]), dim=0)
        train_y = torch.cat((y_train_folds[:i]+y_train_folds[i+1:]), dim=0)
        val_x = x_train_folds[i]
        val_y = y_train_folds[i]

        model = KnnClassifier(train_x, train_y)
        acc_list.append(model.check_accuracy(val_x, val_y, k))
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for k, acc_list in sorted(k_to_accuracies.items(), reverse=True):
